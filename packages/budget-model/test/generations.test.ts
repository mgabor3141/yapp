...
import { extractMajorVersion, findCheapestInGenerations } from "../src/index.js";
...
describe("findCheapestInGenerations", () => {
	describe("anthropic", () => {
		it("generations=1 picks from latest gen only", () => {
			const anthropic = modelsForProvider(allModels, "anthropic");
			const result = findCheapestInGenerations(anthropic, 1);
		...
		it("generations=2 includes previous gen — cheaper haiku at $0.25", () => {
			const anthropic = modelsForProvider(allModels, "anthropic");
			const result = findCheapestInGenerations(anthropic, 2);
			const gens = new Set(result.map((m) => extractMajorVersion(m.id)));
		...
		it("generations=0 includes all generations", () => {
			const anthropic = modelsForProvider(allModels, "anthropic");
			const all = findCheapestInGenerations(anthropic, 0);
	...
	describe("openai", () => {
		it("generations=1 picks from gen 5 — nano at $0.05", () => {
			const openai = modelsForProvider(allModels, "openai");
			const result = findCheapestInGenerations(openai, 1);

			expect(result[0].id).toBe("gpt-5-nano");
			expect(result[0].cost.input).toBe(0.05);
		});

		it("generations=2 adds gen 4 models but nano is still cheapest", () => {
			const openai = modelsForProvider(allModels, "openai");
			const result = findCheapestInGenerations(openai, 2);
			const gens = new Set(result.map((m) => extractMajorVersion(m.id)));
	...
	describe("google", () => {
		it("generations=1 finds free model in latest gen", () => {
			const google = modelsForProvider(allModels, "google");
			const result = findCheapestInGenerations(google, 1);
		...
		it("generations=2 adds gen 2 models", () => {
			const google = modelsForProvider(allModels, "google");
			const result = findCheapestInGenerations(google, 2);
			const gens = new Set(result.map((m) => extractMajorVersion(m.id)));
	...
	describe("xai", () => {
		it("generations=1 picks grok-4 fast variants at $0.2", () => {
			const xai = modelsForProvider(allModels, "xai");
			const result = findCheapestInGenerations(xai, 1);
			expect(result[0].cost.input).toBe(0.2);
			expect(extractMajorVersion(result[0].id)).toBe(4);
		});

		it("generations=2 adds gen 3 — grok-3-mini at $0.3 is more expensive", () => {
			const xai = modelsForProvider(allModels, "xai");
			const result = findCheapestInGenerations(xai, 2);
	...
	describe("edge cases", () => {
		it("returns empty for no versioned models", () => {
			...
			expect(findCheapestInGenerations(unversioned, 1)).toEqual([]);
		});

		it("returns empty for empty input", () => {
			expect(findCheapestInGenerations([], 1)).toEqual([]);
		});

		it("generations larger than available returns same as generations=0", () => {
			const anthropic = modelsForProvider(allModels, "anthropic");
			const all = findCheapestInGenerations(anthropic, 0);
			const huge = findCheapestInGenerations(anthropic, 100);
